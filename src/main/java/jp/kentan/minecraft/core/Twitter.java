...
public class Twitter {
	...
	public Twitter(NekoCore neko, ConfigManager config, EconomyManager eco) {
		...
		standbyStatusList.clear();
		failedStatusList.clear();
		...
		twitter.addListener(new TwitterAdapter() {
			...
			public void onException(TwitterException e, TwitterMethod method) {
				...
				if ((errCode == 88 || errCode == 131 || errCode == 185) && statusUpdate != null) {
					...
					failedStatusList.add(statusUpdate);
					NekoCore.LOG.warning("数分語に再試行します.");
				}
			}
		});
	...
	public void timerHandler(){
		if(failedStatusList.size() > 0 && ++cntTimer >= 6){
			...
			failedStatusList.remove(0);
			return;
		}
		
		if(standbyStatusList.size() > 0){
			twitter.updateStatus(statusUpdate = standbyStatusList.get(0));
			standbyStatusList.remove(0);
		}
	}
	...
	private boolean isReplay(Status status) {
		if (status.getInReplyToScreenName().equals("DekitateServer")
				&& !status.getUser().getScreenName().equals("DekitateServer")) {
			return true;
		}
		return false;
	}
